...
try:
    ...
    chardet = None
ALLOWED = {
    'frontend': ['React.js', 'React.tsx', 'Next.js', 'Vue', 'Angular'],
    'backend':  ['Django', 'FastAPI', 'Express', 'NestJS', 'Spring Boot'],
    'sql':      ['PostgreSQL', 'MySQL', 'MariaDB', 'SQLite', 'MSSQL'],
    'ai':       ['Gemini', 'DeepSeek', 'GPT', 'Claude', 'Naver Clova'],
}

def _normalize_choice(name, kind, variant=None):
    if not name:
        return None
    s = str(name).lower().strip()
    if kind == 'frontend':
        if variant and str(variant).lower().strip() == 'react-tsx':
            return 'React.tsx'
        if 'next' in s: return 'Next.js'
        if 'react' in s: return 'React.tsx'
        if 'vue' in s: return 'Vue'
        if 'angular' in s: return 'Angular'
    if kind == 'backend':
        if 'django' in s: return 'Django'
        if 'fastapi' in s: return 'FastAPI'
        if 'nestjs' in s or 'nest' in s: return 'NestJS'
        if 'express' in s: return 'Express'
        if 'spring' in s: return 'Spring Boot'
    if kind == 'sql':
        if 'postgre' in s or 'postgres' in s: return 'PostgreSQL'
        if 'mysql' in s: return 'MySQL'
        if 'maria' in s: return 'MariaDB'
        if 'sqlite' in s: return 'SQLite'
        if 'mssql' in s or 'sql server' in s: return 'MSSQL'
    if kind == 'ai':
        ...
        if 'clova' in s or 'naver' in s: return 'Naver Clova'
    # 허용 목록과 완전 일치 시
    for a in ALLOWED[kind]:
        if s == a.lower():
            return a
    return None

def _heuristic_from_text(spec_text: str, plan_text: str):
    t = f"{spec_text or ''}\n{plan_text or ''}".lower()
    fe = 'Next.js' if any(k in t for k in ['next','ssr','seo']) else \
         'Vue' if 'vue' in t else \
         'Angular' if 'angular' in t else \
         'React.tsx'
    be = 'Django' if any(k in t for k in ['django','orm','admin','python']) else \
         'FastAPI' if 'fastapi' in t else \
         'NestJS' if ('nestjs' in t or 'nest' in t) else \
         'Spring Boot' if ('spring' in t or 'java' in t) else \
         'Express'
    sq = 'PostgreSQL' if any(k in t for k in ['timeseries','event','cdc','etl','warehouse','analytics','gis','postgis']) else \
         'SQLite' if ('sqlite' in t or 'mobile' in t) else \
         'MariaDB' if 'maria' in t else \
         'MSSQL' if ('mssql' in t or 'sql server' in t) else \
         'MySQL' if 'mysql' in t else \
         'PostgreSQL'
    ai = 'DeepSeek' if 'deepseek' in t else \
         'GPT' if ('openai' in t or 'gpt' in t) else \
         'Claude' if 'claude' in t else \
         'Naver Clova' if ('clova' in t or 'naver' in t) else \
         'Gemini'
    return {'frontend': fe, 'backend': be, 'sql': sq, 'ai': ai}

def _pick_allowed(kind, v):
    return v if v in ALLOWED[kind] else ALLOWED[kind][0]

def enforce_allowlist_on_summary(summary_obj: dict, spec_md: str, plan_md: str) -> dict:
    """
    build_recommendation_summary() 결과(summary_obj)를 받아
    - 허용 후보(5종 목록) 밖 값/누락을 정규화
    - 전부 비었으면 텍스트 히ュー리스틱으로 채움
    - 그래도 비면 ENV 기본값으로 채움
    스키마 가정: summary_obj = { 'frontend': {'recommendation': str|None, 'reason': str|None}, ... }
    """
    out = json.loads(json.dumps(summary_obj))  # deepcopy
    keys = ['frontend','backend','sql','ai']

    # 1) 전부 비었는지 체크
    all_empty = all(not (out.get(k) or {}).get('recommendation') for k in keys)
    if all_empty:
        h = _heuristic_from_text(spec_md, plan_md)
        for k in keys:
            out.setdefault(k, {})
            out[k]['recommendation'] = h[k]
            out[k]['reason'] = out[k].get('reason') or '키워드 히ュー리스틱'

    # 2) 카테고리별 정규화 (허용 후보 밖/애매표현 정리)
    out['frontend']['recommendation'] = _normalize_choice(out['frontend'].get('recommendation'), 'frontend')
    out['backend']['recommendation']  = _normalize_choice(out['backend'].get('recommendation'),  'backend')
    out['sql']['recommendation']      = _normalize_choice(out['sql'].get('recommendation'),      'sql')
    out['ai']['recommendation']       = _normalize_choice(out['ai'].get('recommendation'),       'ai')

    # 3) 누락은 ENV 기본값으로 채움(그래도 허용목록 안으로 강제)
    defv_fe = os.environ.get('DEFAULT_FRONTEND', 'React.tsx')
    defv_be = os.environ.get('DEFAULT_BACKEND',  'Django')
    defv_sq = os.environ.get('DEFAULT_SQL',      'PostgreSQL')
    defv_ai = os.environ.get('DEFAULT_AI',       'Gemini')
    defaults = {'frontend': defv_fe, 'backend': defv_be, 'sql': defv_sq, 'ai': defv_ai}
    for k in keys:
        if not out[k].get('recommendation'):
            out[k]['recommendation'] = _pick_allowed(k, defaults[k])
            out[k]['reason'] = out[k].get('reason') or '빈/불일치 응답 → 기본값 적용'
        else:
            # 허용목록 밖인 경우 첫 후보로 대체
            out[k]['recommendation'] = _pick_allowed(k, out[k]['recommendation'])
...
def main():
    ...
    if interactive_mode:
        ...
        try:
            ...
            summary_obj = enforce_allowlist_on_summary(summary_obj, spec_md, plan_md)
