...
export async function generateFrontend({ plan }: Input) {
  const artifactDir = process.env.ARTIFACT_DIR || path.resolve(process.cwd(), "artifacts/frontend");
...
async function getFilemap(planText: string, kind: "frontend") {
