...
export async function generateBackend({ plan }: Input) {
  const artifactDir = process.env.ARTIFACT_DIR || path.resolve(process.cwd(), "artifacts/backend");
  await fs.ensureDir(artifactDir);

  const filemap = await getFilemap(plan, "backend");
...
async function getFilemap(planText: string, kind: "backend") {
  const sys = "당신은 백엔드 코드 구조를 파일 단위로 설계합니다. JSON만 출력하세요.";
  const usr = `다음 계획을 백엔드 파일맵(JSON: {\n  \"files\": [ { \"path\": string, \"brief\": string } ]\n})으로 변환:\n\n${planText}`;
